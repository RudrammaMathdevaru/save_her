...
const ForgotPasswordPage = () => {
  ...
  const handleChange = useCallback((e) => {
    ...
    setSubmitError("");
  }, [touchedFields, validateField, calculatePasswordStrength, formData.confirmPassword]);
  ...
  // Handle email submission (verify email exists)
  const handleEmailSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      setSubmitError("Unable to verify email. Please try again.");
    } finally {
      setIsLoading(false);
    }
  }, [formData.email, validateField, resetAttempts, isRateLimited]);
  ...
  }, [formData.password, formData.confirmPassword, validateField, navigate]);
  ...
  const inputWrapperClass = useMemo(() => (fieldName) => {
    const baseClasses = "flex items-center gap-3 rounded-xl border px-4 py-3.5 transition-all duration-200";
    ...
    const disabledClasses = isLoading || isRateLimited ? "opacity-60 pointer-events-none" : "";
  ...
  const getPasswordStrengthInfo = useCallback(() => {
    ...
    if (passwordStrength < 70) return { color: "bg-yellow-500", text: "Medium" };
    ...
    <>
      <div className="mb-6 text-center">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Enter your email address to reset your password. If an account exists, you'll be able to set a new password.
        </p>
      </div>

      <form onSubmit={handleEmailSubmit} noValidate aria-label="Email verification form">
        <div className="mb-6">
          <label 
            htmlFor="email"
  ...
  const renderResetStep = () => (
    <>
      <div className="mb-6 text-center">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Create a new password for <span className="font-medium text-indigo-600">{verifiedEmail}</span>
        </p>
      </div>

      <form onSubmit={handleResetSubmit} noValidate aria-label="Password reset form">
        {/* Password Field */}
        <div className="mb-4">
          <label 
            htmlFor="password"
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-white/70 backdrop-blur-sm z-20">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-sm font-medium text-indigo-600">
                    {flowStep === 'email' ? "Verifying..." : "Resetting password..."}
                  </p>
                </div>
              </div>
            )}
